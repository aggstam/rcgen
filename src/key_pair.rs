...
impl KeyPair {
	...
	pub(crate) fn from_raw(
		...
		let (kind, alg) = if let Ok(edkp) = Ed25519KeyPair::from_pkcs8_maybe_unchecked(pkcs8) {
			...
			EcdsaKeyPair::from_pkcs8(&signature::ECDSA_P256_SHA256_ASN1_SIGNING, pkcs8, &system_random)
		{
			(KeyPairKind::Ec(eckp), &PKCS_ECDSA_P256_SHA256)
		} else if let Ok(eckp) =
			EcdsaKeyPair::from_pkcs8(&signature::ECDSA_P384_SHA384_ASN1_SIGNING, pkcs8, &system_random)
		{
	...
	pub fn generate(alg: &'static SignatureAlgorithm) -> Result<Self, RcgenError> {
		...
		match alg.sign_alg {
			SignAlgo::EcDsa(sign_alg) => {
				...
				let key_pair =
					EcdsaKeyPair::from_pkcs8(&sign_alg, &&key_pair_doc.as_ref(), &system_random).unwrap();
				Ok(KeyPair {
